...
class PoseRewardWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        agent_keypoints = get_walker2d_keypoints(self.env)
        ...
        keypoint_mse = float(np.mean((agent_keypoints - ref_keypoints) ** 2))

        info = dict(info)
        info["env_reward"] = float(env_reward)
        info["pose_reward"] = float(imitation_reward)
        info["keypoint_mse"] = keypoint_mse
