...
def main():
    ...
    # Keep only the desired training budget.
    all_results = all_results[all_results["total_steps"] == args.total_steps].copy()

    # Prefer v2 clean run if both old and v2 exist, because v2 has explicit schema.
    all_results["prefer_rank"] = all_results["run_dir"].str.contains("_v2").astype(int)

    # Keep one run per condition/seed. If duplicates exist, keep preferred run.
    all_results = (
        all_results
        .sort_values(["noise_type", "jitter_sigma", "seed", "prefer_rank", "run_dir"])
        .drop_duplicates(["noise_type", "jitter_sigma", "seed"], keep="last")
    ...
    all_results = all_results[cols].sort_values(["noise_type", "jitter_sigma", "seed"])
    ...
    grouped = (
        all_results
        .groupby(["noise_type", "jitter_sigma"], as_index=False)
        .agg(
            n=("seed", "count"),
        ...
        .sort_values(["noise_type", "jitter_sigma"])
    )
